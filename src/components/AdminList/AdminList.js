...
const AdminList = (props) => {
  ...
  const handleDelete = (e, id)=>{
    ...
    return (
        <Grid container style={{padding:'10px 5px', background: '#e9e6e6', borderRadius: '5px',  margin: '5px 0'}} alignItems="center">
        <Grid sm={2} md={2}>{name}</Grid>
    <Grid sm={2} md={3}>{username}</Grid>
    <Grid sm={2} md={2}>{newDate}</Grid>
    <Grid sm={2} md={4}>{event}</Grid>
    <Grid sm={2} md={1}>
